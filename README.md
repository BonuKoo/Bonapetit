...
**실행 계획** — 개발(H2)에서 재고, 운영(MySQL)에서 다시 쟀더니 **결과가 갈렸습니다.**

| 쿼리 | H2 (방 234건) | MySQL (방 5,000건 / 총 11만 건) |
|---|---|---|
| 첫 페이지 | 방 전체 읽음 (`scanCount: 235`) | **실제 51행** (`Backward index scan`) |
| 스크롤 구간 | 인덱스 범위 (`scanCount: 101`) | **실제 51행** |

H2 결과만 봤을 때는 "첫 페이지가 방 크기에 비례해 느려진다"가 메시지 캐싱의 근거였습니다. **MySQL에서 재측정하니 그 근거가 사라졌습니다.** MySQL은 복합 인덱스를 역방향으로 스캔하며 `LIMIT`에서 조기 종료하므로 방 크기와 무관하게 51행만 읽습니다. H2에서 본 전체 스캔은 **H2 옵티마이저의 한계**였습니다.

**복합 인덱스는 여전히 값을 합니다** (MySQL, 방 22개 · 11만 건):
...
인덱스가 없으면 PK를 역방향으로 훑으며 `room_id`가 맞지 않는 행을 계속 버립니다. 51건을 얻으려 1,016행을 읽고 965행을 버립니다.

**결론적으로 캐시의 실질적 가치는 인가 3쿼리 제거**입니다. 메시지 캐싱은 왕복 1회를 더 줄이는 부수 효과입니다. 개발 DB만 보고 결론 냈다면 근거를 잘못 세웠을 사례입니다.
...
| [작업 기록](docs/worklog-2026-08.md) | 2026-08 채팅 영속화 · 설정 분리 · 캐시 작업과 검증 데이터 |
| [이어가기 프롬프트](docs/handoff.md) | **새 작업자·새 세션이 바로 이어받을 수 있는 인수인계 문서** |
