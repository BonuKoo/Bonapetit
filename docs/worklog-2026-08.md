...
| 기간 | 2026-08-31 |
| 대상 | `master` |
| 결과 | PR [#69](https://github.com/BonuKoo/Bonapetit/pull/69) · [#70](https://github.com/BonuKoo/Bonapetit/pull/70) · [#71](https://github.com/BonuKoo/Bonapetit/pull/71) 머지, [#72](https://github.com/BonuKoo/Bonapetit/pull/72) 검토 중 |
...
| 테스트 | `contextLoads()`조차 실패, 외부 의존 3종 필요 | **50건 통과**, 외부 의존 없이 실행 |
...
| 인덱스 | `(room_id, chat_message_id)` | 스크롤 구간에서 범위 조건으로 활용됨 (측정 결과는 [6.3](#63-mysql-재측정--결론이-뒤집힌-지점) 참조) |
...
"채팅방에 들어왔을 때 빠르게 읽고 싶다"는 요구에서 출발했으나, **캐싱 전에 무엇이 비용인지부터 측정**했습니다.

### 6.1 쿼리 개수 측정

코드를 읽어 세는 것과 실제 실행은 다를 수 있습니다. 예컨대 `ChatRoom.team`은 `@OneToOne` 기본값이 EAGER라 조인으로 함께 오는지 별도 SELECT가 나가는지 읽기만으로는 확정할 수 없습니다. Hibernate `Statistics`로 실제 실행되는 쿼리를 셌습니다.
...
**4건 중 3건이 인가 검증**이라는 것이 측정으로 확인됐습니다. 그리고 4건이라는 숫자 자체가 `team`이 **조인으로 온다는 증거**입니다(별도 SELECT였다면 5건).

`ChatHistoryQueryCountTest` 4건이 이 수치를 회귀 테스트로 고정합니다. 연관관계 페치 전략이나 인가 로직이 바뀌어 쿼리가 늘면 여기서 걸립니다.

### 6.2 실행 계획 측정 (H2)

실사용 데이터 234건에 `EXPLAIN ANALYZE`를 돌렸습니다.

```
-- 스크롤 구간 (before 있음)
/* IDX_CHAT_MESSAGE_ROOM_ID: ROOM_ID = '...' AND CHAT_MESSAGE_ID < 100 */
/* scanCount: 101 */          ← 인덱스가 범위를 좁힘. 설계대로

-- 첫 페이지 (before 없음)
/* IDX_CHAT_MESSAGE_ROOM_ID: ROOM_ID = '...' */
/* scanCount: 235 */          ← 방의 메시지 234건을 전부 읽음
```

H2에서는 **첫 페이지가 방 전체를 스캔했습니다.** `WHERE` 조건이 `ROOM_ID`뿐이라 `ORDER BY id DESC LIMIT 51`을 만족시키려 그 방의 메시지를 다 읽고 정렬합니다. 복합 인덱스를 강제해도 같았습니다.

이 시점의 판단은 "첫 페이지가 방 크기에 비례해 느려지므로 캐싱 근거가 강하다"였습니다. **그러나 MySQL 재측정에서 뒤집혔습니다.**

### 6.3 MySQL 재측정 — 결론이 뒤집힌 지점

개발 DB(H2)만 보고 결론 내는 것이 위험하다고 판단해, MySQL 8.4 컨테이너에 Hibernate로 동일 스키마를 만들고 데이터를 채워 재측정했습니다.
...
**H2에서 관측한 전체 스캔은 H2 옵티마이저의 한계였지 쿼리 설계 문제가 아니었습니다.** MySQL은 복합 인덱스를 역방향으로 스캔하며 `LIMIT`에서 조기 종료하므로 방 크기와 무관하게 51행만 읽습니다.

#### 복합 인덱스의 실제 기여 (MySQL, 방 22개 · 11만 건 · 한 방 4.5%)

| 조건 | 실제 읽은 행 | 시간 |
|---|---|---|
| `idx_chat_message_room_id` 사용 | **51** | 0.116 ms |
| `PRIMARY`만 강제 | **1,016** | 0.623 ms |

인덱스가 없으면 PK를 역방향으로 훑으며 `room_id`가 맞지 않는 행을 계속 버립니다. 51건을 얻으려 1,016행을 읽고 **965행을 버립니다**. 방이 많아질수록 이 낭비가 커집니다.

데이터가 한쪽으로 치우쳤을 때(한 방이 테이블의 97%)는 옵티마이저가 `PRIMARY`를 고르기도 했습니다. 그 분포에서는 PK 역스캔으로도 금세 51건이 차기 때문입니다. **현실적인 분포에서는 복합 인덱스를 정확히 선택합니다.**

#### 캐시 설계에 미친 영향

메시지 캐싱의 성능 근거는 **사라졌습니다.** 운영 환경에서 메시지 조회는 방 크기와 무관하게 51행·0.1ms 수준입니다.

따라서 **캐시의 실질적 가치는 인가 검증 3쿼리를 없애는 데 있습니다.** 메시지 캐싱은 왕복 1회를 더 줄이는 부수 효과이며 그 자체로는 큰 이득이 아닙니다. 다만 이미 구현이 끝났고 실패 격리도 갖췄으므로 유지합니다. 제거하면 인가 캐시만 남기는 셈인데, 그 경우 여전히 메시지 쿼리 1건이 남습니다.

> **배운 것** — 개발 DB에서만 측정하고 결론 냈다면 근거를 잘못 세웠을 사례입니다. H2와 MySQL은 같은 인덱스에 대해 다른 실행 계획을 냅니다.
>
> 부수적으로, Hibernate가 FK용으로 만드는 `ROOM_ID` 단일 인덱스는 **H2에서만** 나타납니다. MySQL은 복합 인덱스의 접두사가 FK를 커버하므로 중복 인덱스를 만들지 않습니다.

### 6.4 설계

| 키 | 자료구조 | 용도 | TTL |
|---|---|---|---|
| `CHAT_RECENT:{roomId}` | List (최대 101) | 방 진입 시 읽는 최신 메시지 | 1일, 접근 시 갱신 |
| `CHAT_AUTH:{roomId}:{oauth2Id}` | String | 멤버십 검증 결과 | 5분 |

| 결정 | 근거 |
|---|---|
| 첫 페이지만 캐싱 | 캐시/DB 경계가 `before` 유무 하나로 명확해져, 두 소스에 걸친 커서 계산을 피한다 |
| 용량 101 | size 상한 100 + 다음 페이지 판정용 1건. 100이면 `size=100` 요청만 캐시를 못 타는 구멍이 생긴다 |
| LPUSHX로 갱신 | 키가 없는데 push하면 반쪽짜리 캐시가 생겨 이후 조회가 "더 이전 메시지가 없다"고 잘못 판단한다. 캐시는 반드시 통째로 채운다 |
| 멤버만 캐싱 | 비멤버까지 캐싱하면 방금 참여한 사용자가 TTL 동안 차단된다 |
| 즉시 무효화 | 탈퇴·강퇴는 권한을 빼앗는 작업이라 `PostTeamService.kickMember`에서 바로 지운다. TTL 5분은 그 경로를 놓쳤을 때의 안전망 |

### 6.5 실패 정책 — 영속화와 반대

메시지 영속화는 **정합성 우선**이라 저장 실패 시 발행까지 막습니다. 반면 **캐시는 가용성 우선**이라 모든 Redis 오류를 삼키고 캐시 미스 또는 무동작으로 처리합니다. 예외가 전파되면 Redis 장애가 곧 채팅 조회 장애가 됩니다.

특히 **멤버십 조회 실패는 반드시 `false`를 반환**해야 합니다. `true`면 Redis 장애가 인가 우회로 이어집니다. `ChatCacheRepositoryTest`의 장애 격리 6건이 이 계약을 고정합니다.

### 6.6 문서 정정

측정 결과에 따라 앞서 작성한 문서를 고쳤습니다. README와 `architecture.md`에 **"커서 쿼리가 복합 인덱스만으로 처리된다"**고 썼는데, 그건 스크롤 구간에만 해당하고 첫 페이지는 아닙니다. 해당 서술을 정정하고 측정 근거를 함께 실었습니다.
