...
| 인덱스 | `(room_id, chat_message_id)` | 스크롤 구간에서 범위 조건으로 활용됨 (측정 결과는 [6.2](#62-실행-계획-측정--예상이-깨진-지점) 참조) |
...
**첫 페이지 쿼리는 방 전체를 스캔합니다.** `WHERE` 조건이 `ROOM_ID`뿐이라 `ORDER BY id DESC LIMIT 51`을 만족시키려 그 방의 메시지를 다 읽고 정렬합니다. 복합 인덱스를 강제해도 같았습니다.

이 결과로 판단이 뒤집혔습니다. 측정 전에는 "메시지 조회는 이미 인덱스로 처리되는 가장 빠른 쿼리라 캐싱 효과가 작다"고 봤는데, **반대였습니다.**

- 첫 페이지는 방 크기에 비례해 선형으로 느려집니다
- 하필 그게 **방 진입 시 실행되는 바로 그 쿼리**입니다
- 234건에서 235 스캔이면 10,000건 방에서는 10,000 스캔입니다

인가 3건을 없애는 것과 **별개로 메시지 캐싱 자체의 근거**가 측정에서 나왔습니다.

> **측정 환경의 한계** — H2 기준입니다. MySQL은 같은 인덱스로 역방향 스캔을 하며 51건에서 조기 종료할 수 있어 결과가 다를 수 있습니다. 운영 환경 재측정이 필요합니다.
>
> 또 Hibernate가 FK용으로 만든 `ROOM_ID` 단일 인덱스가 복합 인덱스의 접두사와 겹쳐 **중복**입니다. H2가 첫 페이지에서 이쪽을 고르기도 합니다.
