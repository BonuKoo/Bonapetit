...
| 🔴 치명 | 0 | — |
| 🟠 높음 | 2 | ISS-02, ISS-03 |
| 🟡 중간 | 6 | ISS-04, ISS-05, ISS-06, ISS-08, ISS-09, ISS-10 |
| ⚪ 낮음 | 4 | ISS-07, ISS-13, ISS-14, ISS-17 |
| ✅ 해소 | 5 | ISS-01, ISS-11, ISS-12, ISS-15, ISS-16 |
...
## ISS-11
**보안 · 설계 · 🟡 중간 · ✅ 2026-09-01 해소**

### 서술을 두 군데 정정합니다

조치 전에 재 봤더니 **원래 서술이 두 곳 틀렸습니다.**

| 원래 서술 | 실측 |
|---|---|
| "링크 프리페치나 **크롤러 접근만으로** 삭제가 발생할 수 있습니다" | 크롤러는 삭제하지 못합니다. `@PreAuthorize("hasRole('ROLE_ADMIN')")`가 걸려 있습니다. 실제 위험 대상은 **관리자 세션**입니다 |
| "오류 응답에 **스택 정보가 포함될 수 있습니다**" | 포함되지 않습니다. Spring Boot 3 기본값이 `include-stacktrace=never`·`include-message=never`이고 이 프로젝트는 그 설정을 바꾸지 않았습니다 |

`ErrorResponseProbeTest`가 두 번째를 고정합니다.

### 실제로 있던 문제 셋

| 문제 | 조치 |
|---|---|
| 공지 삭제가 `GET`([API-37](api.md#6-공지)) | `POST`로 변경. 화면의 링크도 폼 전송으로 |
| 없는 id의 상태 코드가 **500** | `GlobalExceptionHandler` — `IllegalArgumentException` → 400, `NoSuchElementException` → 404 |
| 비로그인 사용자가 오류를 만나면 **로그인 페이지로 튕김**(302) | `/error`를 `permitAll`로. 오류 디스패치까지 보안 필터에 걸리고 있었습니다 |

세 번째는 재다가 나왔습니다. `/post/detail/999999`를 부르면 400도 500도 아닌 **302 → `/login`**이 돌아왔습니다.

전역 처리기는 **위 두 예외만** 다룹니다. `AccessDeniedException`은 Spring Security가 403으로 처리하므로 가로채면 안 되고, 나머지는 Boot 기본 처리(500)가 맞으며 내부 정보도 새지 않습니다. 포괄 처리기를 두면 예상 못 한 예외를 조용히 삼킬 위험만 생깁니다.

### 남은 것 — CSRF

**CSRF 비활성은 그대로입니다.** → [ISS-17](#iss-17)

## ISS-17
**보안 · ⚪ 낮음**

CSRF가 전역으로 비활성입니다(`SecurityConfig`의 `.csrf(AbstractHttpConfigurer::disable)`).

[ISS-11](#iss-11) 조치로 상태 변경이 전부 `POST`가 되어 링크·이미지 태그로 실행되는 경로는 막혔습니다. 하지만 공격자 페이지의 **자동 제출 폼**은 여전히 통합니다.

**켜지 않은 이유** — 켜는 것 자체는 한 줄이지만 **브라우저에서 확인할 수 없는 변경**이 셋 딸려 옵니다.

| 대상 | 필요한 것 |
|---|---|
| Thymeleaf 폼 | `th:action`이 토큰을 자동으로 넣습니다. 추가 작업 없음 |
| `axios.post` 2곳(`/chat/enter`, `/team/join`) | axios가 `XSRF-TOKEN` 쿠키를 읽어 헤더로 보내려면 `CookieCsrfTokenRepository.withHttpOnlyFalse()`가 필요합니다 |
| SockJS 폴백 전송 | WebSocket이 안 되는 환경에서 `xhr_send` 등으로 POST합니다. `/ws-stomp/**`를 CSRF 예외로 두지 않으면 **그 환경의 채팅이 끊깁니다** |

여기에 Spring Security 6의 `XorCsrfTokenRequestAttributeHandler` 동작까지 얽혀, **소셜 로그인을 마친 브라우저로 직접 확인하지 않고 넣으면 안 되는 변경**입니다. 자동화된 클라이언트가 로그인할 수 없어([ISS-09](#iss-09)) 지금은 그 확인이 불가능합니다.

**착수 조건** — ISS-09를 먼저 해소하거나, 실제 브라우저로 소셜 로그인 후 세 경로를 직접 확인할 수 있을 때.
...
> **2026-09-01 부하 측정 후** — 착수 순위가 더 내려갔습니다. 첫 측정에서 **DB는 병목이 아니었습니다**(인메모리 H2인데도 포화). DB 왕복을 줄이는 개선은 효과가 크지 않을 가능성이 높습니다. → [채팅 발송 부하 측정](experiments/2026-09-chat-send-load.md)
...
| ~~-~~ | ~~**ISS-11** GET 삭제·오류 상태 코드~~ | ✅ 2026-09-01 해소. CSRF는 [ISS-17](#iss-17)로 분리 |
