...
> 이 목록은 숨기지 않고 기록하는 것을 원칙으로 합니다. 문서에 "동작한다"고 적힌 기능이 실제로는 동작하지 않는 상태가 가장 나쁘기 때문입니다.
...
| 🔴 치명 | 1 | ISS-01 |
| 🟠 높음 | 2 | ISS-02, ISS-03 |
| 🟡 중간 | 6 | ISS-04, ISS-05, ISS-06, ISS-08, ISS-10, ISS-11, ISS-12 |
| ⚪ 낮음 | 2 | ISS-07, ISS-09 |
...
모임 수정·삭제·강퇴([API-20/21/23](api.md#4-모임)), 참여 해제([API-12](api.md#3-프로필)), 채팅 구독([WS-03](api.md#stomp-채널))에 소유자·멤버십 검증이 없습니다.

- `PostController.updateTeam` · `deleteTeam` · `kickMember` — 요청자가 해당 모임의 개설자인지 확인하지 않음
- `AccountProfileController.leaveTeam` — `account_id`를 파라미터로 받고 세션 주체와 대조하지 않음
- `StompHandler` SUBSCRIBE — 구독 시 방 참여자 여부를 확인하지 않음

**영향** — 로그인한 사용자면 타인의 모임을 임의로 조작하거나, `roomId`만 알면 타 모임 대화를 실시간으로 열람할 수 있습니다.

**참고** — `ChatHistoryService.verifyMembership`에 이미 멤버십 검증 패턴이 구현되어 있어 재사용 가능합니다.
...
| 순위 | 대상 | 근거 |
|---|---|---|
| 1 | **ISS-01** 인가 검증 추가 | 이용자 데이터에 직접 피해가 발생할 수 있는 유일한 항목. 채팅 내역 조회에 이미 구현된 멤버십 검증 패턴을 재사용 가능 |
| 2 | **ISS-03** 키 재발급 및 분리 | 노출이 진행 중인 상태. 코드 수정과 콘솔 재발급을 함께 수행해야 함 |
| 3 | **ISS-02** 권한 체계 통일 | 관리자 기능이 예측 불가능하게 동작. 문자열 상수 통일 + 인증 객체 재구성 로직 수정 |
| 4 | **ISS-08** 설정 키 위치 교정 | 수정 비용이 낮고 운영 포트·세션 타임아웃이 정상화됨. 로그 레벨 적용 시 출력량 급증에 유의 |
| 5 | **ISS-12, ISS-06** | 성능 개선. 부하 측정 후 착수 판단 |

---

관련 문서: [요구사항 명세](requirements.md) · [시스템 명세](architecture.md) · [API 명세](api.md)
